...
export function wrapPromptForPremiumEducationalImage(rawPrompt: string): string {
  return [
    ...
    "",
    "TEXT POLICY — STRICT (text in AI images is the #1 quality complaint):",
    "- DEFAULT to ZERO text. Communicate everything visually: shapes, color coding, arrows, spatial position, anatomical icons.",
    "- ONLY add a label if the structure is genuinely indistinguishable without it (e.g. two identical hormone molecules where one is FSH and the other LH). Even then: MAX 2 labels per image, 1 word each.",
    "- Universal abbreviations OK when essential: DNA, RNA, ATP, NH3, H2O, CO2, pH, ECG, ALT, AST, Latin anatomical names (Latin rarely misspells).",
    "- BANNED: full sentences, multi-word phrases, captions, image titles, descriptions, instructions, made-up acronyms, decorative pseudo-text, randomly placed letters, watermark-looking text, fake citations, fake URLs. No Spanish.",
    "- BANNED: stray text in corners/edges (always comes out misspelled), text inside arrows, text floating on background, text near margin.",
    "- If you'd need any pt-BR word with accents (ã, ç, ó, é, ê, í, ú, â, ô) → omit it entirely. Show the concept visually instead.",
    "- When in doubt: NO TEXT. A clean wordless illustration always beats one with garbled labels.",
    "",

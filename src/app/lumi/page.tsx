...
function LumiAssistant({ user }: { user: User }) {
  ...
  const handleQuickAction = useCallback(
    async (action: QuickAction) => {
      ...
      const prompts: Record<
        Exclude<QuickAction["id"], "english">,
        string
      > = {
        summary: `Gere um RESUMO ESTRUTURADO em markdown${subjectHint}${lectureHint}. Use H2 com numeração, bullets, **bold** em termos-chave e termine com "Pontos-chave de revisão".`,
        flashcards: `Crie 8 FLASHCARDS de revisão${subjectHint}${lectureHint}. Formato em markdown: numere cada card como "**Card N — Pergunta:** ..." seguido de "**Resposta:** ...". Inclua dica e dificuldade.`,
        quiz: `Crie um QUIZ com 5 questões de múltipla escolha${subjectHint}${lectureHint}. Cada questão deve ter 4 alternativas, indicar a correta com ✅ e trazer uma breve explicação.`,
        explain: `Explique o conceito mais importante${subjectHint}${lectureHint} de forma didática: definição, mecanismo, exemplo clínico/prático e armadilha comum.`,
      };
      const prompt = prompts[action.id as Exclude<QuickAction["id"], "english">];
      if (!prompt) return;
  ...
  return (
    <div className="relative mx-auto flex max-w-7xl flex-col gap-6 px-4 py-6 lg:px-8">
      {/* Header */}
      ...
      >
        {/* Chat column */}
        <div className="flex min-h-[480px] flex-col rounded-2xl border border-border/60 bg-card">
          <div
            ref={scrollRef}
